export interface NewsArticle {
  id: string;
  title: string;
  summary: string;
  content: string;
  author: string;
  category: string;
  publishedAt: string;
  imageUrl: string;
  readTime: number;
  tags: string[];
  featured?: boolean;
}

export const newsArticles: NewsArticle[] = [
  {
    id: '1',
    title: 'హైదరాబాద్‌లో మెట్రో రైలు నిర్మాణం పూర్తయింది',
    summary: 'హైదరాబాద్ మెట్రో రైలు నిర్మాణం పూర్తయి, ప్రజలకు సులభమైన రవాణా సౌకర్యం లభించింది.',
    content: `హైదరాబాద్‌లో మెట్రో రైలు నిర్మాణం విజయవంతంగా పూర్తయింది. ఈ ప్రాజెక్ట్ ద్వారా నగర ప్రజలకు ఆధునిక మరియు సులభమైన రవాణా సౌకర్యం లభించింది.

మెట్రో రైలు నిర్మాణంలో మూడు ఫేజ్‌లలో మొత్తం 69 కిలోమీటర్ల మార్గం నిర్మించబడింది. ఈ ప్రాజెక్ట్‌లో హైదరాబాద్ మెట్రో రైల్ లిమిటెడ్ (HMRL) మరియు L&T మెట్రో రైల్ హైదరాబాద్ లిమిటెడ్ సంస్థలు కలిసి పనిచేశాయి.

"ఈ మెట్రో రైలు నిర్మాణం హైదరాబాద్ నగరానికి ఒక మైలురాయి," అని ముఖ్యమంత్రి కె. చంద్రశేఖర్ రావు పేర్కొన్నారు. "ఇది నగర రవాణా వ్యవస్థను మార్చి, ప్రజలకు సమయాన్ని ఆదా చేస్తుంది."

మెట్రో రైలు నిర్మాణంలో ఉపయోగించిన ప్రధాన సాంకేతిక లక్షణాలు:
- ఆధునిక సిగ్నలింగ్ వ్యవస్థ
- ఎలక్ట్రిక్ మల్టిపుల్ యూనిట్ (EMU) రైలు బండ్లు
- స్మార్ట్ కార్డ్ టికెట్ వ్యవస్థ
- ఎయిర్ కండిషనింగ్ సౌకర్యం
- డిజిటల్ డిస్ప్లే బోర్డులు

ఈ మెట్రో రైలు నిర్మాణం ద్వారా రోజుకు సుమారు 15 లక్షల ప్రజలు ప్రయాణించగలరు. ఇది రోడ్ ట్రాఫిక్‌ని తగ్గించి, వాయు కాలుష్యాన్ని తగ్గించడానికి సహాయపడుతుంది.

మెట్రో రైలు నిర్మాణంలో ఉపయోగించిన మెటీరియల్స్ మరియు సాంకేతిక పరిజ్ఞానం అన్నీ భారతీయ సంస్థల ద్వారా సరఫరా చేయబడ్డాయి. ఇది 'మేక్ ఇన్ ఇండియా' ఉద్యమానికి ఒక ఉదాహరణ.

భవిష్యత్తులో మెట్రో రైలు నెట్‌వర్క్‌ని మరింత విస్తరించే ప్రణాళికలు ఉన్నాయి. ఇందులో హైదరాబాద్ మెట్రోపాలిటన్ ఏరియా (HMA) లోని ఇతర ప్రాంతాలకు కూడా మెట్రో రైలు సౌకర్యం కల్పించే లక్ష్యం ఉంది.`,
    author: 'రాజేష్ కుమార్',
    category: 'business',
    publishedAt: '2024-01-19T14:30:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1545454675-3531b543be5d?w=800&h=400&fit=crop',
    readTime: 6,
    tags: ['మెట్రో రైలు', 'హైదరాబాద్', 'రవాణా', 'అభివృద్ధి'],
    featured: true,
  },
  {
    id: '2',
    title: 'ఆంధ్రప్రదేశ్‌లో కొత్త టెక్నాలజీ పార్క్ ప్రారంభం',
    summary: 'విశాఖపట్నంలో కొత్త టెక్నాలజీ పార్క్ ప్రారంభించబడింది, ఇది యువతకు ఉపాధి అవకాశాలు కల్పిస్తుంది.',
    content: `ఆంధ్రప్రదేశ్‌లో విశాఖపట్నంలో కొత్త టెక్నాలజీ పార్క్ విజయవంతంగా ప్రారంభించబడింది. ఈ టెక్నాలజీ పార్క్ ద్వారా రాష్ట్రంలోని యువతకు ఉపాధి అవకాశాలు మరియు టెక్నాలజీ అభివృద్ధికి అవకాశం లభిస్తుంది.

ఈ టెక్నాలజీ పార్క్‌లో 50 కంపెనీలు స్థాపించబడ్డాయి, వీటిలో సాఫ్ట్‌వేర్ డెవలప్మెంట్, ఆర్టిఫిషియల్ ఇంటెలిజెన్స్, మరియు డేటా సైన్స్ కంపెనీలు ఉన్నాయి. ఈ పార్క్‌లో సుమారు 5,000 మంది యువతకు ఉపాధి అవకాశం లభిస్తుంది.

"ఈ టెక్నాలజీ పార్క్ ఆంధ్రప్రదేశ్ యువతకు ఒక బంగారు అవకాశం," అని ముఖ్యమంత్రి వై.ఎస్. జగన్మోహన్ రెడ్డి పేర్కొన్నారు. "ఇది రాష్ట్ర ఆర్థిక వ్యవస్థను బలపరుస్తుంది మరియు యువతకు ఉత్తమ భవిష్యత్తును కల్పిస్తుంది."

టెక్నాలజీ పార్క్‌లో ఉన్న ప్రధాన సౌకర్యాలు:
- అధునాతన కంప్యూటర్ ల్యాబ్స్
- ఇంటర్నెట్ కనెక్టివిటీ
- మీటింగ్ రూమ్స్
- కాఫీ షాప్‌లు
- పార్కింగ్ సౌకర్యం
- సెక్యూరిటీ వ్యవస్థ

ఈ టెక్నాలజీ పార్క్‌లో ఉద్యోగులకు ఇచ్చే ప్రధాన ప్రయోజనాలు:
- ఆధునిక టెక్నాలజీ అనుభవం
- ఉత్తమ జీతం మరియు ప్రయోజనాలు
- కెరీర్ అభివృద్ధి అవకాశాలు
- వర్క్-లైఫ్ బాలెన్స్

ఈ టెక్నాలజీ పార్క్‌లో ఉద్యోగులకు ఇచ్చే ప్రధాన ప్రయోజనాలు:
- ఆధునిక టెక్నాలజీ అనుభవం
- ఉత్తమ జీతం మరియు ప్రయోజనాలు
- కెరీర్ అభివృద్ధి అవకాశాలు
- వర్క్-లైఫ్ బాలెన్స్

భవిష్యత్తులో ఈ టెక్నాలజీ పార్క్‌ని మరింత విస్తరించే ప్రణాళికలు ఉన్నాయి. ఇందులో మరిన్ని కంపెనీలను ఆకర్షించడం మరియు ఎక్కువ మంది యువతకు ఉపాధి అవకాశాలు కల్పించడం ఉంది.`,
    author: 'సుధా రెడ్డి',
    category: 'technology',
    publishedAt: '2024-01-18T11:15:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=800&h=400&fit=crop',
    readTime: 5,
    tags: ['టెక్నాలజీ పార్క్', 'విశాఖపట్నం', 'ఉపాధి', 'యువత'],
    featured: false,
  },
  {
    id: '3',
    title: 'తెలంగాణలో కొత్త విద్యా విధానం ప్రకటన',
    summary: 'తెలంగాణ ప్రభుత్వం కొత్త విద్యా విధానాన్ని ప్రకటించింది, ఇది విద్యార్థులకు మెరుగైన విద్యా అవకాశాలు కల్పిస్తుంది.',
    content: `తెలంగాణ ప్రభుత్వం కొత్త విద్యా విధానాన్ని ప్రకటించింది. ఈ విధానం ద్వారా విద్యార్థులకు మెరుగైన విద్యా అవకాశాలు మరియు భవిష్యత్ కెరీర్ అభివృద్ధికి అవకాశం లభిస్తుంది.

కొత్త విద్యా విధానంలో ప్రధాన మార్పులు:
- డిజిటల్ ఎడ్యుకేషన్ పై ఎక్కువ దృష్టి
- ఆర్ట్స్ మరియు సైన్స్ సబ్జెక్టుల మధ్య సంతులనం
- ప్రాక్టికల్ నాలెడ్జ్ పై ఎక్కువ శ్రద్ధ
- ఇంగ్లీష్ మరియు తెలుగు భాషల మధ్య సంతులనం
- స్పోర్ట్స్ మరియు కళలకు ప్రోత్సాహం

"ఈ కొత్త విద్యా విధానం మన విద్యార్థులకు ఉత్తమ భవిష్యత్తును కల్పిస్తుంది," అని విద్యా మంత్రి పేర్కొన్నారు. "ఇది డిజిటల్ యుగంలో విద్యార్థులను సిద్ధం చేస్తుంది."

కొత్త విద్యా విధానంలో ఉన్న ప్రధాన లక్షణాలు:
- స్మార్ట్ క్లాస్‌రూమ్స్
- డిజిటల్ లైబ్రరీలు
- ఆన్‌లైన్ లెర్నింగ్ ప్లాట్‌ఫార్మ్స్
- స్కిల్ డెవలప్మెంట్ ప్రోగ్రామ్స్
- ఇంటర్నేషనల్ ఎడ్యుకేషన్ పార్టనర్‌షిప్స్

ఈ విధానం ద్వారా విద్యార్థులకు లభించే ప్రయోజనాలు:
- ఆధునిక టెక్నాలజీ అనుభవం
- ఇంటర్నేషనల్ స్టాండర్డ్స్
- మెరుగైన ఉపాధి అవకాశాలు
- సమగ్ర వ్యక్తిత్వ అభివృద్ధి

భవిష్యత్తులో ఈ విద్యా విధానాన్ని మరింత మెరుగుపరచే ప్రణాళికలు ఉన్నాయి. ఇందులో మరిన్ని స్కూల్స్ మరియు కళాశాలలకు ఈ సౌకర్యాలను విస్తరించడం ఉంది.`,
    author: 'లక్ష్మి దేవి',
    category: 'politics',
    publishedAt: '2024-01-17T09:45:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1523050854058-8df90110c9e1?w=800&h=400&fit=crop',
    readTime: 4,
    tags: ['విద్యా విధానం', 'తెలంగాణ', 'డిజిటల్ ఎడ్యుకేషన్', 'విద్యార్థులు'],
    featured: false,
  },
  {
    id: '4',
    title: 'హైదరాబాద్‌లో కొత్త సినిమా థియేటర్‌లు ప్రారంభం',
    summary: 'హైదరాబాద్‌లో ఐమ్యాక్స్ మరియు 4DX సినిమా థియేటర్‌లు ప్రారంభించబడ్డాయి, ప్రేక్షకులకు అద్భుతమైన అనుభవం.',
    content: `హైదరాబాద్‌లో ఐమ్యాక్స్ మరియు 4DX సినిమా థియేటర్‌లు విజయవంతంగా ప్రారంభించబడ్డాయి. ఈ థియేటర్‌లు ప్రేక్షకులకు అద్భుతమైన సినిమా అనుభవాన్ని కల్పిస్తాయి.

ఈ కొత్త థియేటర్‌లలో ఉన్న ప్రధాన లక్షణాలు:
- ఐమ్యాక్స్ స్క్రీన్ - 70mm డిజిటల్ ప్రొజెక్షన్
- 4DX సీటింగ్ - మోషన్ సీట్స్ మరియు స్పెషల్ ఎఫెక్ట్స్
- డాల్బీ అట్మాస్ సౌండ్ సిస్టమ్
- 4K డిజిటల్ ప్రొజెక్షన్
- కంఫర్టబుల్ రీక్లైనింగ్ సీట్స్

"ఈ థియేటర్‌లు హైదరాబాద్ ప్రేక్షకులకు ఒక కొత్త అనుభవాన్ని కల్పిస్తాయి," అని థియేటర్ మేనేజర్ పేర్కొన్నారు. "ఇవి ఇంటర్నేషనల్ స్టాండర్డ్స్‌తో సమానమైన సౌకర్యాలను అందిస్తాయి."

ఈ థియేటర్‌లలో ప్రదర్శించబడే సినిమాలు:
- హాలీవుడ్ బ్లాక్‌బస్టర్స్
- బాలీవుడ్ హిట్స్
- తెలుగు సినిమాలు
- డాక్యుమెంటరీలు
- స్పెషల్ షోలు

థియేటర్‌లలో ఉన్న అదనపు సౌకర్యాలు:
- ఫుడ్ కోర్ట్
- గేమింగ్ జోన్
- కిడ్స్ ప్లే ఏరియా
- డిజిటల్ టికెట్ బుకింగ్
- ఆన్‌లైన్ ఫుడ్ ఆర్డరింగ్

ఈ థియేటర్‌లు రోజుకు సుమారు 5,000 మంది ప్రేక్షకులకు సేవ చేయగలవు. ఇవి హైదరాబాద్‌లోని ఎంటర్టైన్మెంట్ సెక్టర్‌కు ఒక కొత్త డైమెన్షన్‌ని జోడిస్తాయి.`,
    author: 'ప్రియా శర్మ',
    category: 'entertainment',
    publishedAt: '2024-01-16T16:20:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1489599835382-957593cb2375?w=800&h=400&fit=crop',
    readTime: 3,
    tags: ['సినిమా థియేటర్', 'ఐమ్యాక్స్', '4DX', 'హైదరాబాద్'],
    featured: false,
  },
  {
    id: '5',
    title: 'ఆంధ్రప్రదేశ్‌లో క్రికెట్ అకాడమీ ప్రారంభం',
    summary: 'విశాఖపట్నంలో కొత్త క్రికెట్ అకాడమీ ప్రారంభించబడింది, యువ క్రికెటర్లకు శిక్షణ అవకాశం.',
    content: `ఆంధ్రప్రదేశ్‌లో విశాఖపట్నంలో కొత్త క్రికెట్ అకాడమీ విజయవంతంగా ప్రారంభించబడింది. ఈ అకాడమీ యువ క్రికెటర్లకు ప్రొఫెషనల్ శిక్షణ అవకాశాన్ని కల్పిస్తుంది.

ఈ క్రికెట్ అకాడమీలో ఉన్న ప్రధాన సౌకర్యాలు:
- అంతర్జాతీయ స్టాండర్డ్స్‌తో కూడిన క్రికెట్ గ్రౌండ్
- నెట్ ప్రాక్టీస్ ఫెసిలిటీలు
- ఫిట్‌నెస్ సెంటర్
- స్పోర్ట్స్ మెడిసిన్ క్లినిక్
- హోస్టల్ సౌకర్యం
- డైటీషన్ సెంటర్

"ఈ అకాడమీ మన యువ క్రికెటర్లకు ఒక బంగారు అవకాశం," అని క్రికెట్ అకాడమీ డైరెక్టర్ పేర్కొన్నారు. "ఇది భవిష్యత్ క్రికెట్ స్టార్లను తయారు చేస్తుంది."

అకాడమీలో ఇచ్చే శిక్షణ కోర్సులు:
- బ్యాటింగ్ టెక్నిక్స్
- బౌలింగ్ టెక్నిక్స్
- ఫీల్డింగ్ డ్రిల్స్
- ఫిట్‌నెస్ ట్రైనింగ్
- మెంటల్ స్ట్రెంగ్త్ ట్రైనింగ్
- మ్యాచ్ అనాలిసిస్

ఈ అకాడమీలో శిక్షణ పొందే విద్యార్థులకు లభించే ప్రయోజనాలు:
- ప్రొఫెషనల్ కోచ్ల నుండి శిక్షణ
- ఇంటర్నేషనల్ క్రికెటర్లతో ఇంటరాక్షన్
- టూర్నమెంట్స్‌లో పాల్గొనే అవకాశం
- స్కాలర్‌షిప్ అవకాశాలు

భవిష్యత్తులో ఈ అకాడమీని మరింత విస్తరించే ప్రణాళికలు ఉన్నాయి. ఇందులో మరిన్ని గ్రౌండ్లు మరియు ట్రైనింగ్ ఫెసిలిటీలు జోడించడం ఉంది.`,
    author: 'రవి కుమార్',
    category: 'sports',
    publishedAt: '2024-01-15T13:10:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&h=400&fit=crop',
    readTime: 4,
    tags: ['క్రికెట్ అకాడమీ', 'విశాఖపట్నం', 'యువ క్రికెటర్లు', 'శిక్షణ'],
    featured: false,
  },
  {
    id: '6',
    title: 'తెలంగాణలో స్మార్ట్ సిటీ ప్రాజెక్ట్‌లు',
    summary: 'తెలంగాణలోని వివిధ నగరాలలో స్మార్ట్ సిటీ ప్రాజెక్ట్‌లు విజయవంతంగా అమలు చేయబడుతున్నాయి.',
    content: `తెలంగాణలోని వివిధ నగరాలలో స్మార్ట్ సిటీ ప్రాజెక్ట్‌లు విజయవంతంగా అమలు చేయబడుతున్నాయి. ఈ ప్రాజెక్ట్‌లు నగర ప్రజలకు మెరుగైన జీవన నాణ్యతను కల్పిస్తాయి.

స్మార్ట్ సిటీ ప్రాజెక్ట్‌లలో ఉన్న ప్రధాన లక్షణాలు:
- స్మార్ట్ ట్రాఫిక్ మేనేజ్మెంట్ సిస్టమ్
- స్మార్ట్ స్ట్రీట్ లైటింగ్
- వేస్ట్ మేనేజ్మెంట్ సిస్టమ్
- స్మార్ట్ వాటర్ సప్లై
- డిజిటల్ గవర్నెన్స్ ప్లాట్‌ఫార్మ్
- స్మార్ట్ పార్కింగ్ సిస్టమ్

"ఈ స్మార్ట్ సిటీ ప్రాజెక్ట్‌లు మన నగరాలను డిజిటల్ యుగంలో సిద్ధం చేస్తాయి," అని ముఖ్యమంత్రి పేర్కొన్నారు. "ఇవి ప్రజల జీవితాలను సులభతరం చేస్తాయి."

స్మార్ట్ సిటీ ప్రాజెక్ట్‌లలో ఉపయోగించిన టెక్నాలజీలు:
- ఇంటర్నెట్ ఆఫ్ థింగ్స్ (IoT)
- ఆర్టిఫిషియల్ ఇంటెలిజెన్స్ (AI)
- బిగ్ డేటా అనాలిసిస్
- క్లౌడ్ కంప్యూటింగ్
- మొబైల్ అప్లికేషన్స్

ఈ ప్రాజెక్ట్‌లు ద్వారా ప్రజలకు లభించే ప్రయోజనాలు:
- మెరుగైన రవాణా వ్యవస్థ
- సమర్థవంతమైన వేస్ట్ మేనేజ్మెంట్
- మెరుగైన భద్రతా వ్యవస్థ
- డిజిటల్ సేవలు
- శక్తి సంరక్షణ

భవిష్యత్తులో ఈ స్మార్ట్ సిటీ ప్రాజెక్ట్‌లను మరిన్ని నగరాలకు విస్తరించే ప్రణాళికలు ఉన్నాయి.`,
    author: 'సురేష్ రెడ్డి',
    category: 'technology',
    publishedAt: '2024-01-14T10:30:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=800&h=400&fit=crop',
    readTime: 5,
    tags: ['స్మార్ట్ సిటీ', 'తెలంగాణ', 'డిజిటల్ గవర్నెన్స్', 'టెక్నాలజీ'],
    featured: false,
  },
  {
    id: '7',
    title: 'ఆంధ్రప్రదేశ్‌లో కొత్త వ్యవసాయ విధానం',
    summary: 'ఆంధ్రప్రదేశ్ ప్రభుత్వం కొత్త వ్యవసాయ విధానాన్ని ప్రకటించింది, రైతులకు మెరుగైన సహాయం.',
    content: `ఆంధ్రప్రదేశ్ ప్రభుత్వం కొత్త వ్యవసాయ విధానాన్ని ప్రకటించింది. ఈ విధానం ద్వారా రైతులకు మెరుగైన సహాయం మరియు ఆదాయం కల్పించే లక్ష్యం ఉంది.

కొత్త వ్యవసాయ విధానంలో ఉన్న ప్రధాన లక్షణాలు:
- డిజిటల్ వ్యవసాయ పద్ధతులు
- ఆర్గానిక్ ఫార్మింగ్ ప్రోత్సాహం
- మైక్రో ఇరిగేషన్ సిస్టమ్స్
- సోయిల్ హెల్త్ కార్డ్స్
- డిజిటల్ మార్కెటింగ్ ప్లాట్‌ఫార్మ్
- ఫార్మర్ ప్రొడ్యూసర్ ఆర్గనైజేషన్స్ (FPOs)

"ఈ కొత్త వ్యవసాయ విధానం మన రైతులకు ఒక కొత్త భవిష్యత్తును కల్పిస్తుంది," అని వ్యవసాయ మంత్రి పేర్కొన్నారు. "ఇది వ్యవసాయ రంగాన్ని ఆధునీకరిస్తుంది."

వ్యవసాయ విధానంలో ఉన్న ప్రధాన సౌకర్యాలు:
- సబ్సిడీలు మరియు రుణాలు
- ఇన్సూరెన్స్ కవరేజ్
- టెక్నికల్ సపోర్ట్
- మార్కెట్ లింకేజ్
- ట్రైనింగ్ ప్రోగ్రామ్స్

ఈ విధానం ద్వారా రైతులకు లభించే ప్రయోజనాలు:
- మెరుగైన ఉత్పాదకత
- ఎక్కువ ఆదాయం
- డిజిటల్ సేవలు
- మార్కెట్ అక్సెస్
- ఇన్సూరెన్స్ కవరేజ్

భవిష్యత్తులో ఈ వ్యవసాయ విధానాన్ని మరింత మెరుగుపరచే ప్రణాళికలు ఉన్నాయి.`,
    author: 'వెంకటేష్ రెడ్డి',
    category: 'business',
    publishedAt: '2024-01-13T08:15:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&h=400&fit=crop',
    readTime: 4,
    tags: ['వ్యవసాయ విధానం', 'ఆంధ్రప్రదేశ్', 'రైతులు', 'డిజిటల్ వ్యవసాయం'],
    featured: false,
  },
  {
    id: '16',
    title: 'ఆంధ్రప్రదేశ్‌లో కొత్త వ్యవసాయ విధానం',
    summary: 'ఆంధ్రప్రదేశ్ ప్రభుత్వం కొత్త వ్యవసాయ విధానాన్ని ప్రకటించింది, రైతులకు మెరుగైన సహాయం.',
    content: `ఆంధ్రప్రదేశ్ ప్రభుత్వం కొత్త వ్యవసాయ విధానాన్ని ప్రకటించింది. ఈ విధానం ద్వారా రైతులకు మెరుగైన సహాయం మరియు ఆదాయం కల్పించే లక్ష్యం ఉంది.

కొత్త వ్యవసాయ విధానంలో ఉన్న ప్రధాన లక్షణాలు:
- డిజిటల్ వ్యవసాయ పద్ధతులు
- ఆర్గానిక్ ఫార్మింగ్ ప్రోత్సాహం
- మైక్రో ఇరిగేషన్ సిస్టమ్స్
- సోయిల్ హెల్త్ కార్డ్స్
- డిజిటల్ మార్కెటింగ్ ప్లాట్‌ఫార్మ్
- ఫార్మర్ ప్రొడ్యూసర్ ఆర్గనైజేషన్స్ (FPOs)

"ఈ కొత్త వ్యవసాయ విధానం మన రైతులకు ఒక కొత్త భవిష్యత్తును కల్పిస్తుంది," అని వ్యవసాయ మంత్రి పేర్కొన్నారు. "ఇది వ్యవసాయ రంగాన్ని ఆధునీకరిస్తుంది."

వ్యవసాయ విధానంలో ఉన్న ప్రధాన సౌకర్యాలు:
- సబ్సిడీలు మరియు రుణాలు
- ఇన్సూరెన్స్ కవరేజ్
- టెక్నికల్ సపోర్ట్
- మార్కెట్ లింకేజ్
- ట్రైనింగ్ ప్రోగ్రామ్స్

ఈ విధానం ద్వారా రైతులకు లభించే ప్రయోజనాలు:
- మెరుగైన ఉత్పాదకత
- ఎక్కువ ఆదాయం
- డిజిటల్ సేవలు
- మార్కెట్ అక్సెస్
- ఇన్సూరెన్స్ కవరేజ్

భవిష్యత్తులో ఈ వ్యవసాయ విధానాన్ని మరింత మెరుగుపరచే ప్రణాళికలు ఉన్నాయి.`,
    author: 'వెంకటేష్ రెడ్డి',
    category: 'business',
    publishedAt: '2024-01-13T08:15:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&h=400&fit=crop',
    readTime: 4,
    tags: ['వ్యవసాయ విధానం', 'ఆంధ్రప్రదేశ్', 'రైతులు', 'డిజిటల్ వ్యవసాయం'],
    featured: false,
  },
  {
    id: '17',
    title: 'తెలంగాణలో కొత్త హెల్త్ కేర్ సెంటర్లు ప్రారంభం',
    summary: 'తెలంగాణలోని వివిధ జిల్లాలలో కొత్త హెల్త్ కేర్ సెంటర్లు ప్రారంభించబడ్డాయి, ప్రజలకు మెరుగైన వైద్య సేవలు.',
    content: `తెలంగాణలోని వివిధ జిల్లాలలో కొత్త హెల్త్ కేర్ సెంటర్లు విజయవంతంగా ప్రారంభించబడ్డాయి. ఈ సెంటర్లు ప్రజలకు మెరుగైన వైద్య సేవలను అందిస్తాయి.

కొత్త హెల్త్ కేర్ సెంటర్లలో ఉన్న ప్రధాన సౌకర్యాలు:
- ప్రాథమిక వైద్య పరీక్షలు
- డిజిటల్ హెల్త్ రికార్డ్స్
- టెలిమెడిసిన్ సేవలు
- ఇమర్జెన్సీ కేర్
- మాతృ మరియు శిశు సంరక్షణ
- ఇమ్యూనైజేషన్ సేవలు

"ఈ హెల్త్ కేర్ సెంటర్లు మన ప్రజలకు నాణ్యమైన వైద్య సేవలను అందిస్తాయి," అని ఆరోగ్య మంత్రి పేర్కొన్నారు. "ఇవి గ్రామీణ ప్రాంతాలలో ఆరోగ్య సంరక్షణను మెరుగుపరుస్తాయి."

హెల్త్ కేర్ సెంటర్లలో అందించే సేవలు:
- సాధారణ వైద్య పరీక్షలు
- రక్త పరీక్షలు
- ఎక్స్-రే సేవలు
- ఫిజియోథెరపీ
- మానసిక ఆరోగ్య సేవలు
- పోషకాహార సలహాలు

ఈ సెంటర్లు ద్వారా ప్రజలకు లభించే ప్రయోజనాలు:
- సులభమైన వైద్య సేవలు
- తక్కువ ఖర్చు
- సమయానికి చికిత్స
- నాణ్యమైన సంరక్షణ
- డిజిటల్ రికార్డ్స్

భవిష్యత్తులో ఈ హెల్త్ కేర్ సెంటర్లను మరిన్ని ప్రాంతాలకు విస్తరించే ప్రణాళికలు ఉన్నాయి.`,
    author: 'అనితా దేవి',
    category: 'politics',
    publishedAt: '2024-01-12T14:20:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1559757148-5c350d0d3c56?w=800&h=400&fit=crop',
    readTime: 5,
    tags: ['హెల్త్ కేర్', 'తెలంగాణ', 'వైద్య సేవలు', 'గ్రామీణ ఆరోగ్యం'],
    featured: false,
  },
  {
    id: '18',
    title: 'ఆంధ్రప్రదేశ్‌లో కొత్త టూరిజం ప్రాజెక్ట్‌లు',
    summary: 'ఆంధ్రప్రదేశ్‌లో కొత్త టూరిజం ప్రాజెక్ట్‌లు ప్రారంభించబడ్డాయి, పర్యాటకులకు అద్భుతమైన అనుభవం.',
    content: `ఆంధ్రప్రదేశ్‌లో కొత్త టూరిజం ప్రాజెక్ట్‌లు విజయవంతంగా ప్రారంభించబడ్డాయి. ఈ ప్రాజెక్ట్‌లు పర్యాటకులకు అద్భుతమైన అనుభవాన్ని కల్పిస్తాయి.

కొత్త టూరిజం ప్రాజెక్ట్‌లలో ఉన్న ప్రధాన లక్షణాలు:
- హెరిటేజ్ టూరిజం డెవలప్మెంట్
- ఎకో టూరిజం ప్రాజెక్ట్‌లు
- అడ్వెంచర్ టూరిజం సెంటర్లు
- హోమ్ స్టే అకమోడేషన్
- డిజిటల్ టూరిజం ప్లాట్‌ఫార్మ్
- కల్చరల్ టూరిజం ప్రోగ్రామ్స్

"ఈ టూరిజం ప్రాజెక్ట్‌లు మన రాష్ట్రానికి ఎక్కువ పర్యాటకులను ఆకర్షిస్తాయి," అని టూరిజం మంత్రి పేర్కొన్నారు. "ఇవి స్థానిక ఆర్థిక వ్యవస్థకు మద్దతు ఇస్తాయి."

టూరిజం ప్రాజెక్ట్‌లలో ఉన్న ప్రధాన సౌకర్యాలు:
- మోడరన్ హోటల్స్ మరియు రిసార్ట్స్
- ట్రాన్స్పోర్ట్ సేవలు
- గైడ్ సేవలు
- ఫుడ్ కోర్ట్స్
- షాపింగ్ సెంటర్లు
- ఎంటర్టైన్మెంట్ జోన్లు

ఈ ప్రాజెక్ట్‌లు ద్వారా లభించే ప్రయోజనాలు:
- ఉపాధి అవకాశాలు
- స్థానిక ఆర్థిక వృద్ధి
- సాంస్కృతిక వినిమయం
- ఇన్ఫ్రాస్ట్రక్చర్ అభివృద్ధి
- పర్యాటకుల అనుభవం

భవిష్యత్తులో ఈ టూరిజం ప్రాజెక్ట్‌లను మరిన్ని ప్రాంతాలకు విస్తరించే ప్రణాళికలు ఉన్నాయి.`,
    author: 'రాజేష్ కుమార్',
    category: 'business',
    publishedAt: '2024-01-11T11:30:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=800&h=400&fit=crop',
    readTime: 4,
    tags: ['టూరిజం', 'ఆంధ్రప్రదేశ్', 'పర్యాటకులు', 'ఆర్థిక వృద్ధి'],
    featured: false,
  },
  {
    id: '19',
    title: 'తెలంగాణలో కొత్త స్పోర్ట్స్ అకాడమీలు',
    summary: 'తెలంగాణలోని వివిధ జిల్లాలలో కొత్త స్పోర్ట్స్ అకాడమీలు ప్రారంభించబడ్డాయి, యువతకు శిక్షణ అవకాశం.',
    content: `తెలంగాణలోని వివిధ జిల్లాలలో కొత్త స్పోర్ట్స్ అకాడమీలు విజయవంతంగా ప్రారంభించబడ్డాయి. ఈ అకాడమీలు యువతకు వివిధ క్రీడలలో శిక్షణ అవకాశాన్ని కల్పిస్తాయి.

కొత్త స్పోర్ట్స్ అకాడమీలలో ఉన్న ప్రధాన క్రీడలు:
- క్రికెట్ అకాడమీ
- ఫుట్‌బాల్ అకాడమీ
- బాస్కెట్‌బాల్ అకాడమీ
- బ్యాడ్మింటన్ అకాడమీ
- టెన్నిస్ అకాడమీ
- అథ్లెటిక్స్ అకాడమీ

"ఈ స్పోర్ట్స్ అకాడమీలు మన యువతకు ఒక బంగారు అవకాశం," అని స్పోర్ట్స్ మంత్రి పేర్కొన్నారు. "ఇవి భవిష్యత్ క్రీడా స్టార్లను తయారు చేస్తాయి."

స్పోర్ట్స్ అకాడమీలలో ఉన్న సౌకర్యాలు:
- అంతర్జాతీయ స్టాండర్డ్స్‌తో కూడిన గ్రౌండ్లు
- ట్రైనింగ్ ఫెసిలిటీలు
- ఫిట్‌నెస్ సెంటర్లు
- స్పోర్ట్స్ మెడిసిన్ క్లినిక్లు
- హోస్టల్ సౌకర్యం
- డైటీషన్ సెంటర్లు

ఈ అకాడమీలలో శిక్షణ పొందే విద్యార్థులకు లభించే ప్రయోజనాలు:
- ప్రొఫెషనల్ కోచ్ల నుండి శిక్షణ
- ఇంటర్నేషనల్ క్రీడాకారులతో ఇంటరాక్షన్
- టూర్నమెంట్స్‌లో పాల్గొనే అవకాశం
- స్కాలర్‌షిప్ అవకాశాలు
- కెరీర్ గైడెన్స్

భవిష్యత్తులో ఈ స్పోర్ట్స్ అకాడమీలను మరింత విస్తరించే ప్రణాళికలు ఉన్నాయి.`,
    author: 'సుధా రెడ్డి',
    category: 'sports',
    publishedAt: '2024-01-10T09:15:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&h=400&fit=crop',
    readTime: 4,
    tags: ['స్పోర్ట్స్ అకాడమీలు', 'తెలంగాణ', 'యువత', 'క్రీడలు'],
    featured: false,
  },
  {
    id: '20',
    title: 'ఆంధ్రప్రదేశ్‌లో కొత్త టెక్నాలజీ ఇనిషియేటివ్స్',
    summary: 'ఆంధ్రప్రదేశ్‌లో కొత్త టెక్నాలజీ ఇనిషియేటివ్స్ ప్రారంభించబడ్డాయి, యువతకు ఉపాధి అవకాశాలు.',
    content: `ఆంధ్రప్రదేశ్‌లో కొత్త టెక్నాలజీ ఇనిషియేటివ్స్ విజయవంతంగా ప్రారంభించబడ్డాయి. ఈ ఇనిషియేటివ్స్ యువతకు టెక్నాలజీ రంగంలో ఉపాధి అవకాశాలను కల్పిస్తాయి.

కొత్త టెక్నాలజీ ఇనిషియేటివ్స్‌లో ఉన్న ప్రధాన లక్షణాలు:
- ఆర్టిఫిషియల్ ఇంటెలిజెన్స్ ట్రైనింగ్
- మెషీన్ లెర్నింగ్ కోర్సులు
- డేటా సైన్స్ ప్రోగ్రామ్స్
- సైబర్ సెక్యూరిటీ ట్రైనింగ్
- క్లౌడ్ కంప్యూటింగ్ కోర్సులు
- బ్లాక్‌చైన్ టెక్నాలజీ ట్రైనింగ్

"ఈ టెక్నాలజీ ఇనిషియేటివ్స్ మన యువతకు ఒక కొత్త భవిష్యత్తును కల్పిస్తాయి," అని టెక్నాలజీ మంత్రి పేర్కొన్నారు. "ఇవి డిజిటల్ యుగంలో ఉపాధి అవకాశాలను కల్పిస్తాయి."

టెక్నాలజీ ఇనిషియేటివ్స్‌లో ఉన్న ప్రధాన సౌకర్యాలు:
- మోడరన్ కంప్యూటర్ ల్యాబ్స్
- ఇంటర్నెట్ కనెక్టివిటీ
- డిజిటల్ లైబ్రరీలు
- మెంటర్‌షిప్ ప్రోగ్రామ్స్
- ఇంటర్న్‌షిప్ అవకాశాలు
- ప్లేస్మెంట్ సపోర్ట్

ఈ ఇనిషియేటివ్స్ ద్వారా యువతకు లభించే ప్రయోజనాలు:
- టెక్నాలజీ స్కిల్స్ డెవలప్మెంట్
- ఉపాధి అవకాశాలు
- ఇంటర్నేషనల్ ఎక్స్పోజర్
- ఎంటర్‌ప్రెన్యూర్‌షిప్ అవకాశాలు
- ఇండస్ట్రీ కనెక్షన్స్

భవిష్యత్తులో ఈ టెక్నాలజీ ఇనిషియేటివ్స్‌ని మరిన్ని జిల్లాలకు విస్తరించే ప్రణాళికలు ఉన్నాయి.`,
    author: 'లక్ష్మి దేవి',
    category: 'technology',
    publishedAt: '2024-01-09T16:45:00Z',
    imageUrl: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=800&h=400&fit=crop',
    readTime: 5,
    tags: ['టెక్నాలజీ ఇనిషియేటివ్స్', 'ఆంధ్రప్రదేశ్', 'యువత', 'ఉపాధి'],
    featured: false,
  },
  {
    id: '21',
    title: 'దీక్షా శిబిరం వద్ద మాట్లాడుతున్న కలెక్టర్',
    summary: 'జన్నారంలో బంద్ సక్సెస్ - రిలే నిరాహార దీక్షను సందర్శించిన కలెక్టర్',
    content: `జన్నారంలో బంద్ సక్సెస్ 
రిలే నిరాహార దీక్షను సందర్శించిన కలెక్టర్

జన్నారం, జూలై 25 : కవ్వాల్ అభయారణ్యంలో భారీ వాహనాలను అనుమతించాలని గత పది రోజులుగా మండల కేంద్రంలో గల అంబేద్కర్ విగ్రహం వద్ద సామాజిక కార్యకర్త శ్రీరాముల భూమాచారి, అజ్మీర బద్రీ నాయక్ లు చేపట్టిన రిలే నిరాహార దీక్షకు మంచి స్పందన లభించింది. అన్ని వర్గాల ప్రజలు ఈ రిలే నిరాహార దీక్షకు మద్దతు తెలిపి దీక్షలో కూర్చుంటున్నారు. శుక్రవారం రోజుతో రిలే నిరాహార దీక్ష పదవ రోజుకు చేరుకుంది. రేషన్ కార్డుల పంపిణీ కార్యక్రమానికి వచ్చిన జిల్లా కలెక్టర్ కుమార్ దీపక్ కార్యక్రమాన్ని ముగించుకుని మంచిర్యాల వెళుతుండగా గమనించిన దీక్ష శిబిరాన్ని కలెక్టర్ కుమార్ దీపక్ సందర్శించారు. ఇక్కడి పరిస్థితులు ఏంటని భూమా చారి, బద్రీ నాయక్ లను కలెక్టర్ అడిగి తెలుసుకున్నారు. అటవీ అధికారులు భారీ వాహనాలను కవ్వాల్ టైగర్ జోన్ పరిధిలోకి అనుమతించక పోవడంతో మండల ప్రజలు అనేక ఇబ్బందులకు గురవుతున్నారని, అనేకసార్లు ప్రజాప్రతినిధులకు, అటవీ అధికారులకు విన్నవించినా ఫలితం లేకుండా పోయిందని, దీంతో ఈ రిలే దీక్ష నిర్వహించడం చేపట్టిందని తెలిపారు. అనంతరం కలెక్టర్ కు వినతి పత్రాన్ని అందజేశారు. ఈ విషయంలో అటవీ అధికారులతో మాట్లాడి సమస్యను పరిష్కారమయ్యే విధంగా చూస్తానని కలెక్టర్ హామీ ఇచ్చారు. కలెక్టర్ దీక్ష శిబిరం వద్ద ఆగడంతో పోలీసులు ఒక్కసారిగా ఉలిక్కిపడ్డారు. ఏమి జరుగుతుందోననే ఆలోచనతో లక్షెట్టిపేట సీఐ రమణమూర్తి, జన్నారం, దండేపల్లి ఎస్సైలు గొల్లపల్లి అనూష, తహిషోద్దీన్, పోలీసులు అక్కడికి చేరుకున్నారు.`,
    author: 'స్థానిక విలేఖరి',
    category: 'politics',
    publishedAt: '2025-07-25T10:00:00Z',
    imageUrl: '/images/jnrm.jpeg',
    readTime: 3,
    tags: ['జన్నారం', 'కలెక్టర్', 'నిరాహార దీక్ష', 'కవ్వాల్ అభయారణ్యం', 'అటవీ అధికారులు'],
    featured: true,
  },
  {
    id: '22',
    title: 'జనగామ రోడ్డులో భారీ వర్షానికి చెట్టు విరిగిపడింది – ప్రజలకు తీవ్ర అసౌకర్యం',
    summary: 'మంచిర్యాల జిల్లా జనారం పట్టణంలో భారీ వర్షానికి పెద్ద చెట్టు ప్రధాన రోడ్డుపై విరిగిపడి రహదారి మూసుకుపోయింది.',
    content: `జనగామ రోడ్డులో భారీ వర్షానికి చెట్టు విరిగిపడింది – ప్రజలకు తీవ్ర అసౌకర్యం

మంచిర్యాల జిల్లా జనారం పట్టణంలో జరిగిన భారీ వర్షానికి ఒక పెద్ద చెట్టు ప్రధాన రోడ్డుపై విరిగిపడింది. ఈ ఘటన వలన రహదారి పూర్తిగా మూసుకుపోయింది మరియు వాహనాల రాకపోకలకు తీవ్ర అంతరాయం కలిగింది. చెట్టు పడిన ప్రాంతం చిత్తడి మట్టితో కప్పబడినందున, వాహనదారులు జారిపడే ప్రమాదం ఉంది. ప్రజలు ముప్పు మిట్టలో ఇరుక్కుపోయారు.

ఈ ఘటన వర్షపాతం తీవ్రమై ఉండటం వల్ల జరిగినదని స్థానికులు చెబుతున్నారు. అధికారులు వెంటనే స్పందించి చెట్టు తొలగింపునకు చర్యలు చేపట్టాలని ప్రజలు డిమాండ్ చేస్తున్నారు.

ఈ ఘటన వల్ల ప్రజలకు ఎదురవుతున్న సమస్యలు:
- రహదారి పూర్తిగా మూసుకుపోవడం
- వాహనాల రాకపోకలకు అంతరాయం
- చిత్తడి మట్టి వల్ల జారిపడే ప్రమాదం
- ప్రజలు ముప్పు మిట్టలో ఇరుక్కుపోవడం

స్థానిక ప్రజలు అధికారులను వెంటనే స్పందించి చెట్టు తొలగించి రహదారిని తెరవాలని డిమాండ్ చేస్తున్నారు. ఈ ఘటన వల్ల ప్రజలకు తీవ్ర అసౌకర్యం ఎదురవుతోంది.`,
    author: 'ఎన్. రిషికుమార్ జన్నారం',
    category: 'local',
    publishedAt: '2025-07-26T08:00:00Z',
    imageUrl: '/images/news1.jpeg',
    readTime: 2,
    tags: ['జనారం', 'మంచిర్యాల జిల్లా', 'వర్షం', 'చెట్టు విరిగిపడడం', 'రహదారి అంతరాయం'],
    featured: false,
  },
  {
    id: '23',
    title: 'జనారం పట్టణంలో వర్షపు నీరు తొరుగుదల - ప్రజలకు ఇబ్బందులు',
    summary: 'మంచిర్యాల జిల్లా జనారం పట్టణంలో భారీ వర్షానికి నీరు తొరుగుదల వల్ల ప్రజలకు తీవ్ర ఇబ్బందులు ఎదురవుతున్నాయి.',
    content: `జనారం పట్టణంలో వర్షపు నీరు తొరుగుదల - ప్రజలకు ఇబ్బందులు

మంచిర్యాల జిల్లా జనారం పట్టణంలో భారీ వర్షానికి నీరు తొరుగుదల వల్ల ప్రజలకు తీవ్ర ఇబ్బందులు ఎదురవుతున్నాయి. వర్షపు నీరు తొరుగుదల వల్ల పట్టణంలోని అనేక ప్రాంతాల్లో నీరు నిలిచి ఉంది మరియు ప్రజలకు తీవ్ర అసౌకర్యం ఎదురవుతోంది.

వర్షపు నీరు తొరుగుదల వల్ల ఎదురవుతున్న సమస్యలు:
- పట్టణంలోని అనేక ప్రాంతాల్లో నీరు నిలిచి ఉండడం
- ప్రజలకు ఇళ్లకు వెళ్లడానికి ఇబ్బంది
- వాహనాల రాకపోకలకు అంతరాయం
- వ్యాపార వ్యవహారాలకు ఇబ్బంది

స్థానిక అధికారులు వెంటనే స్పందించి నీరు తొరుగుదలకు చర్యలు చేపట్టాలని ప్రజలు డిమాండ్ చేస్తున్నారు. ఈ ఘటన వల్ల ప్రజలకు తీవ్ర అసౌకర్యం ఎదురవుతోంది.`,
    author: 'ఎన్. రిషికుమార్ జన్నారం',
    category: 'local',
    publishedAt: '2025-07-26T09:00:00Z',
    imageUrl: '/images/news2.jpeg',
    readTime: 2,
    tags: ['జనారం', 'వర్షపు నీరు తొరుగుదల', 'మంచిర్యాల జిల్లా', 'ప్రజల ఇబ్బందులు'],
    featured: false,
  },
  {
    id: '24',
    title: 'ఈషికా మొబైల్ యాప్ ప్రారంభం – సాంప్రదాయ నృత్య శిక్షణ ప్రారంభం',
    summary: 'జన్నారంలో ఈషికా ట్రైనింగ్ అండ్ ప్లేస్మెంట్స్ ఆధ్వర్యంలో స్వయం ఉపాధి వృత్తుల కోసం మొబైల్ యాప్ మరియు సాంప్రదాయ నృత్య శిక్షణ ప్రారంభం.',
    content: `పత్రికా ప్రకటన
తేదీ: 26-07-2025
జారీ చేసిన వారు: ఈషికా ట్రైనింగ్ అండ్ ప్లేస్మెంట్స్, జన్నారం

⸻

చిన్నతరహా స్వయం ఉపాధి వృత్తుల కోసం ఈషికా మొబైల్ యాప్ ప్రారంభం – సాంప్రదాయ నృత్య శిక్షణ ప్రారంభం

జన్నారం లోని ఈషికా ట్రైనింగ్ అండ్ ప్లేస్మెంట్స్ ఆధ్వర్యంలో ప్లంబర్లు, ఎలక్ట్రీషియన్లు, డ్రైవర్లు మరియు ఇతర స్వయం ఉపాధి వృత్తులకు ఉపయోగపడే మొబైల్ యాప్‌ను 26 జూలై 2025న ప్రారంభించారు. ఈ యాప్ ద్వారా కస్టమర్లకు నేరుగా సొంత ప్రాంతంలోని వృత్తిపరుల సేవలు అందుబాటులోకి వస్తాయి. ఇది స్వయం ఉపాధిదారులకు కొత్త అవకాశాలను తెచ్చిపెడుతుంది.

ఈ యాప్ ప్రారంభం rural areas లో డిజిటల్ సదుపాయాలు అందించడంలో ఒక పెద్ద ముందడుగు. ఉద్యోగం కోసం ప్రయత్నిస్తున్న వారికీ, నైపుణ్యాలు ఉన్న వారికీ ఇది గొప్ప వేదికగా నిలవనుంది.

ఈ సందర్భంగా ఈషికా సంస్థలో శాస్త్రోక్త పూజ నిర్వహించి, అనంతరం సాంప్రదాయ నృత్య శిక్షణ తరగతులను ప్రారంభించారు. భారత సాంప్రదాయ నృత్య కళలను అభివృద్ధి పరచడమే ఈ తరగతుల ముఖ్య ఉద్దేశం.

ఈ కార్యక్రమానికి ముఖ్యఅతిథులుగా విచ్చేసిన వారు:
• ముత్యం సతీష్ గారు, ఛైర్మన్, ARS డిగ్రీ కాలేజ్
• కస్తూరి సతీష్ గారు, ఛైర్మన్, శ్రీ కృష్ణవేణి హై స్కూల్
• చెలువేరు సుధీర్ కుమార్ గారు, సూర్య న్యూస్ రిపోర్టర్

ముఖ్య అతిథులు ఈషికా సంస్థ చేపట్టిన డిజిటల్ మరియు సాంస్కృతిక అభివృద్ధి చర్యలను ప్రశంసిస్తూ, గ్రామీణ యువతకు ఆత్మనిర్భరత వైపు తీసుకెళ్లే మంచి ప్రయత్నమని అభినందించారు.

వివరాలకు సంప్రదించండి:
🌐 వెబ్‌సైట్: www.eshikatp.online
📞 ఫోన్: 9575334335
📧 ఇమెయిల్: eshikatrainingplacements@gmail.com`,
    author: 'ఈషికా ట్రైనింగ్ అండ్ ప్లేస్మెంట్స్',
    category: 'business',
    publishedAt: '2025-07-26T10:00:00Z',
    imageUrl: '/images/eshika.jpeg',
    readTime: 4,
    tags: ['ఈషికా', 'మొబైల్ యాప్', 'స్వయం ఉపాధి', 'సాంప్రదాయ నృత్య శిక్షణ', 'జన్నారం'],
    featured: true,
  },
];

export const categories = [
  { name: 'technology', label: 'Technology', color: '#1976d2' },
  { name: 'business', label: 'Business', color: '#2e7d32' },
  { name: 'politics', label: 'Politics', color: '#d32f2f' },
  { name: 'sports', label: 'Sports', color: '#ed6c02' },
  { name: 'entertainment', label: 'Entertainment', color: '#9c27b0' },
  { name: 'local', label: 'Local', color: '#7b1fa2' },
];

export const getArticlesByCategory = (category: string): NewsArticle[] => {
  return newsArticles.filter(article => article.category === category);
};

export const getFeaturedArticles = (): NewsArticle[] => {
  return newsArticles.filter(article => article.featured);
};

export const getArticleById = (id: string): NewsArticle | undefined => {
  return newsArticles.find(article => article.id === id);
};

export const searchArticles = (query: string): NewsArticle[] => {
  const lowercaseQuery = query.toLowerCase();
  return newsArticles.filter(article => 
    article.title.toLowerCase().includes(lowercaseQuery) ||
    article.summary.toLowerCase().includes(lowercaseQuery) ||
    article.content.toLowerCase().includes(lowercaseQuery) ||
    article.tags.some(tag => tag.toLowerCase().includes(lowercaseQuery))
  );
}; 
